...
A partir do desequilíbrio na geração t, $D_t$, podemos calcular a frequência de qualquer gameta subtraindo o desequilíbrio na geração t da frequência do gameta no equilíbrio. Por exemplo:

$s = \hat{s} - D_t$

Implementando função para calcular a desequilíbrio e a frequência gamética em t gerações:
...
calc_D_freq(c = 0.5, t=1:20)

# Frequencias para c=0.4 (locos ligados)
calc_D_freq(c = 0.4, t=1:20)
```
- Locos não ligados:
