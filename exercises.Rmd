...
Como essa média é estimada a partir do ponto médio, devemos somá-la ao valor do ponto médio. Portanto:
...
$S_{total} = \sum\limits_{i=1}^n S_i$
...
$$
\begin{aligned}
\alpha1 &= q[a+d(q-p)] = q \alpha \\
\alpha2 &= -p[a+d(q-p)] = -p \alpha
\end{aligned}
$$
...
$$
\begin{aligned}
A(A1A1) &= 2 \alpha1 = 2q\alpha \\
A(A1A2) &= \alpha1 + \alpha2 = \alpha(q-p) \\
A(A2A2) &= 2 \alpha2 = -2p \alpha
\end{aligned}
$$
...
\begin{aligned}
G(A1A1) &= 2q(\alpha - qd) \\
G(A1A2) &= \alpha (q-p) + 2pqd \\
G(A2A2) &= -2p (\alpha + pd)
\end{aligned}
$$

```{r}
G_a1a1 <- 2 * q * (alpha - q * d)
G_a1a1

G_a1a2 <- alpha * (q-p) + 2*p*q*d
G_a1a2

G_a2a2 <- -2 * p * (alpha + p*d)
G_a2a2
```
...
$$
\begin{aligned}
D(A1A1) &= -2q^2d = G(A1A1) - A(A1A1) \\
D(A1A2) &= 2pqd = G(A1A2) - A(A1A2) \\
D(A2A2) &= -2p^2d = G(A2A2) - A(A2A2)
\end{aligned}
$$
...
resumo_tabela <- data.frame(
  Genotipo = c("A1A1", "A1A2", "A2A2"),
  Codificados = c("+a = 30", "d = 50", "-a = -30"),
  A = c(16, -4, -24),
  G = c(0, 20, -60),
  D = c(-16, 24, -36)
)
